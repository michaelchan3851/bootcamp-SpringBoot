      ...
      <ul class="marquee-content h-full flex" ref="mq">
        ...
        >
          <div class="flex justify-between w-3/4">
            ...
            <div>
              ...
              <!-- percent red -->
              <p class="font-bold text-xs text-red-400 flex justify-end items-center" v-if="$filters.price_negative(coin.price_change_percentage_24h)">
        ...
        >
          <div class="flex justify-between w-3/4">
            ...
            <!-- price -->
            <div>
              <p class="font-bold flex justify-end text-xs lg:text-base">{{ $filters.comma_separator(coin.current_price) }}</p>
              <!-- percent red -->
              <p class="font-bold text-xs text-red-400 flex justify-end items-center" v-if="$filters.price_negative(coin.price_change_percentage_24h)">
                <fa icon="caret-down" class="mr-1" />
                {{ $filters.string_trunc(coin.price_change_percentage_24h, 5) }}%
      ...
      <table class="table-fixed cursor-pointer">
        ...
        <tbody class="divide-y">
          ...
          <tr class="text-sm hover:bg-gray-100 transition duration-300" v-for="coin in matchedNames" :key="coin.name">
            ...
            <td class="font-bold text-gray-600">${{ $filters.comma_separator(coin.current_price) }}</td>
            <td class="font-bold">
...
export default {
  ...
  setup() {
    ...
    const retrieveCoins = async () => {
      ...
      "http://localhost:8085/crypto/api/v1/coin/market"       
    //  "https://api.coingecko.com/api/v3/coins/markets?ids=bitcoin,ethereum,tether,dogecoin,ripple&vs_currency=usd&x_cg_demo_api_key=CG-rLnNTQ6CvY1GKwVvLAnkr29v"   
    //shttps://api.coingecko.com/api/v3/coins/markets?ids=bitcoin,ethereum,tether,dogecoin,ripple&vs_currency=usd&x_cg_demo_api_key=CG-dX4J8rJdGueUK44pe7bWs12K
        );
